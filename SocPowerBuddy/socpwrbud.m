...
static struct param_set metrics_set[] =
{
    ...
    { METRIC_FREQ,     "show active frequencies of unit(s)" },
    { METRIC_DVFM,     "show dvfm state distribution of unit(s)" },
    { METRIC_DVFMMS,   "show time spent in dvfm states during sample of unit(s)" },
    { METRIC_POWER,    "show power consumption of unit(s)" },
    { METRIC_INSTRCTS, "show instructions retired / per-clock metrics of supporting unit(s)" },
    { METRIC_CORES,    "show per-core stats for selected metrics on supporting unit(s)" }
};
...
//        bd.ane  = true;
        ...
        while((opt = getopt_long(argc, argv, "hvi:s:po:m:H:wga", long_opts, &optindex)) != -1) {
            switch(opt) {
                ...
                case 'o': cmd.file_out = fopen(strdup(optarg), "w");
                          break;
                case 'a': cmd.metrics = [NSArray arrayWithObjects:[NSString stringWithUTF8String: METRIC_RES],
                                        [NSString stringWithUTF8String: METRIC_DVFM],
        ...
        for (int i = 0; i < [cmd.metrics count]; i++) {
            if ([cmd.metrics[i] isEqual:[NSString stringWithUTF8String:METRIC_RES]])            bd.res        = true;
