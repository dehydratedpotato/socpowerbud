            ...
            cmd_data* cmd)      // command line arg data (i.e. intervals)
{
     ...
     */
    /*if (iorep->cpusamp_a == NULL)*/ iorep->cpusamp_a = IOReportCreateSamples(iorep->cpusub, iorep->cpusubchn, NULL);
    
   /* if (iorep->pwrsamp_a == NULL)*/ iorep->pwrsamp_a = IOReportCreateSamples(iorep->pwrsub, iorep->pwrsubchn, NULL);
    
    [NSThread sleepForTimeInterval:(cmd->interval*1e-3)];
    
    iorep->cpusamp_b = IOReportCreateSamples(iorep->cpusub, iorep->cpusubchn, NULL);
    iorep->pwrsamp_b = IOReportCreateSamples(iorep->pwrsub, iorep->pwrsubchn, NULL);
    
    /*
     * Iterating through the delta(s) of our CPU/GPU Stat samples (for our frequencies)
     */
    IOReportIterate(IOReportCreateSamplesDelta(iorep->cpusamp_a, iorep->cpusamp_b, NULL), ^(IOReportSampleRef sample) {
     ...
     */
    IOReportIterate(IOReportCreateSamplesDelta(iorep->pwrsamp_a, iorep->pwrsamp_b, NULL), ^(IOReportSampleRef sample) {
        NSString* chann_name = IOReportChannelGetChannelName(sample);
